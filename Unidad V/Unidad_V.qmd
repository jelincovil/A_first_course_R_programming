...
Cuando un dataset es limitado y presenta una **gran cantidad**de valores perdidos, se puede utilizar la **imputación múltiple**. Esta técnica implica ajustar un modelo estadístico (basado en los datos observados) para predecir y generar
múltiples conjuntos de valores *plausibles* para los datos faltantes.

La imputación múltiple rellena los datos faltantes generando varias versiones
completas del conjunto de datos, rellenando los valores perdidos de forma probabilística. Esto permite analizar los datos sin sesgos
significativos por la pérdida de información y combinar los resultados para una inferencia más robusta, incorporando la incertidumbre de la imputación. Es crucial para estudios con patrones de datos faltantes
complejos o no aleatorios.

### Metodología de Rubin

La metodología `mice` transforma un conjunto de datos con valores ausentes en un modelo estadístico final. El proceso aplica las **Reglas de Rubin** para integrar la variabilidad introducida por la imputación en los estimaciones de varios modelos en uno solo resultante.

---

### Pasos Clave de la Metodología `mice`

- Generar Múltiples Imputaciones: Se crean varias versiones completas del conjunto de datos, donde los valores faltantes se rellenan empleando un **proceso aleatorio controlado** para producir diferencias entre las versiones.

- Ajuste/entrenamiento de cada dataset imputado: El modelo estadístico de interés se ajusta individualmente en cada uno de estos conjuntos de datos completos generados.

- Combinar los Resultados (Reglas de Rubin): Los resultados de los modelos ajustados a cada dataset se agrupan. Este paso calcula una estimación combinada y ajusta su varianza para reflejar la variabilidad tanto dentro de cada imputación como entre las imputaciones.

- Usar el Modelo Final: El modelo resultante de la combinación proporciona parámetros y sus errores estándar que incorporan la incertidumbre de los valores originalmente ausentes.
...
#### Rellenar los Datos Faltantes (5 Veces)

![5-folds](https://raw.githubusercontent.com/jelincovil/A_first_course_R_programming/refs/heads/main/images_for_r_programming/mice.png){width=50%}
...
imputed_dataset_5 <- complete(imputed_data, 5)

#View(imputed_dataset_1)
#View(imputed_dataset_5)
...
$$
chl = \beta_0+ \beta_1 bmi + \beta_2 hyp + e
$$
...
# Combinar los resultados de los 5  modelos
modelo_lineal_combinados <- pool(model_fits)

# Mostrar resumen combinado
print(summary(modelo_lineal_combinados))

```
#### Estimación de un modelo logistico con datos imputados

$$
P(hyp=1)= \dfrac{e^{ \beta_0 + \beta_1bmi+ \beta_2chl + \beta_3age } }{1+e^{ \beta_0 + \beta_1bmi+ \beta_2chl + \beta_3age}}.
$$
```{r}
# Codi
library(dplyr)

nhanes_recodificado <- nhanes %>%
                       mutate(hyp = if_else(hyp == 1, 1, 0))


# Ajustar modelo lineal en cada dataset imputado
moderl_fits <- with(nhanes_recodificado, glm(hyp ~ bmi + chl + age, family = "binomial"))

# Combinar los resultados de los 5  modelos
modelo_logistico_combinados <- pool(model_fits)

# Mostrar resumen combinado
print(summary(modelo_logistico_combinados))
```

## **7. Ejemplo de Estudio de Propiedades Estadísticas**

Este principio utiliza la simulación para comprender y verificar el comportamiento asintótico de los estimadores y métodos inferenciales. La simulación nos da una visión empírica vital de su fiabilidad en escenarios prácticos.

#### **Consistencia de un Estimado: Evaluar la Fiabilidad del Estimador de Tasa de Fallas de Componentes**

En ingeniería, estimar la tasa de fallas ($\lambda$) de componentes es crucial para la planificación del mantenimiento. El Estimador de Máxima Verosimilitud (MLE), $1/\bar{x}_n$, es comúnmente usado para datos de tiempo hasta la falla en distribución exponencial. 

$$
X_1,X_2,\ldots, X_n; \quad \bar{X}_n=\frac{1}{n}\sum_{i=1}^{n}X_i, \quad \frac{1}{\bar{X}_n}
$$
$$
P\left( \left|\frac{1}{\bar{X}_n} - \lambda \right|>\epsilon\right) \to 0, \quad \mbox{cuando } n\to \infty
$$

La siguiente simulación verificar empíricamente si este estimador es consistente, es decir, si sus estimaciones se acercan al valor real de la tasa de fallas a medida que se recolectan más datos.
...
```{r}
tamano_n <- seq(10,1000, 10)
estimativa_n <- rep(0, length(tamano_n))

for (i in 1:length(tamano_n)) {
  data_exp <- rexp(tamano_n[i], rate = true_lambda)
  estimativa_n[i] <- 1/mean(data_exp)
}

plot(tamano_n , estimativa_n, type="l",
     xlab="n", ylab="Estimativa")
abline(h=true_lambda, col="red", lwd=2)

```

#### **Propiedades de un Intervalo de Confianza: Verificar el coeficiente de confianza de un IC para el Consumo Promedio en Marketing**
...
$$
![Intervalo](https://raw.githubusercontent.com/jelincovil/A_first_course_R_programming/refs/heads/main/images_for_r_programming/intervalo.png){width=50%}
...
También podemos usar la simulación para verficar visualmente el desempeño de un 
...
# Parámetros
true_mu <- 10            # Media verdadera de la población
true_sigma <- 5          # Desviación estándar verdadera de la población
sample_size <- 20        # Tamaño de muestra por simulación
confidence_level <- 0.95 # Nivel de confianza deseado para los intervalos
n_sims_plot <- 50        # Reducido para el gráfico 

# Almacenar los límites de los ICs y si contienen la media verdadera
plot_data <- data.frame(
  sim_id = 1:n_sims_plot,
  lower_ci = numeric(n_sims_plot),
  upper_ci = numeric(n_sims_plot),
  contains_mu = logical(n_sims_plot)
)

set.seed(789) # Para reproducibilidad del gráfico

for (i in 1:n_sims_plot) {
  sample_data <- rnorm(n = sample_size, mean = true_mu, sd = true_sigma)
  t_test_result <- t.test(sample_data, conf.level = confidence_level)

  plot_data$lower_ci[i] <- t_test_result$conf.int[1]
  plot_data$upper_ci[i] <- t_test_result$conf.int[2]
  plot_data$contains_mu[i] <- (true_mu >= t_test_result$conf.int[1] && true_mu <= t_test_result$conf.int[2])
}

# --- Creación del gráfico de ICs
ggplot(plot_data, aes(x = sim_id, y = (lower_ci + upper_ci) / 2)) +
  geom_errorbar(aes(ymin = lower_ci, ymax = upper_ci, color = contains_mu), width = 0.2, linewidth = 0.8) +
  geom_point(aes(color = contains_mu), size = 1.5) +
  geom_hline(yintercept = true_mu, linetype = "dashed", color = "black", linewidth = 1) +
  labs(
    title = paste0("Intervalos de Confianza del ", confidence_level * 100, "% para la Media"),
    subtitle = paste0("Línea punteada negra: Media Verdadera (", true_mu, ")"),
    x = "Simulación Individual",
    y = "Valor de la Media μ"
  ) +
  scale_color_manual(values = c("TRUE" = "darkgreen", "FALSE" = "red"),
                     labels = c("TRUE" = "Contiene μ", "FALSE" = "No contiene μ")) + theme_minimal() + coord_flip()
...
for (i in 1:n_sims) {
  X1 <- rnorm(n_obs)
  Z <- rnorm(n_obs)
  X2 <- correlation*X1 + sqrt(1 - correlation^2)*Z # X2 correlacionado
...
par(mfrow = c(1, 2))
hist(beta1_estimates, breaks = 30, main = "Estimaciones Beta1", xlab = "Coef. Tamaño de la casa", col = "lightblue")
abline(v = beta1_true, col = "red", lty = 2, lwd = 2)
legend("topright", legend = "Parámetro Verdadero", col = "red", lty = 2, lwd = 2, cex = 0.6)

hist(beta2_estimates, breaks = 30, main = "Estimaciones Beta2", xlab = "Coef. N de habitaciones", col = "lightgreen")
