...
# Simulación/generación de datos desde modelos de probabilidad
...
set.seed(123)
original_sample <- rnorm(n = 50, mean = 50, sd = 10) # 30 tiempos de respuesta
...
-   El promedio es insesgado para estimar \$\\mu\$,
...
set.seed(123) 
boot_results <- boot(data = aSAH, statistic = calcular_auc, R = 10000)
...
lasso_model_cv <- train(x = X, y = Y,
                        method = "glmnet",
                        trControl = ctrl,
                        tuneGrid = expand.grid(alpha = 1, # Regresión tipo Lasso
...
$$
P(``\mbox{Rechazar $H_0$ cuando es verdadera''})= \int_{A}f_{T}(t)dt \leq \alpha.
$$
...
-   Taza: 0.07 , n= 70
-   Taza: 0.06 , n = 200
-   Taza: 0.05, n=1000 Respetar la taza de error.
-   Taza: 0.05, n=5000
